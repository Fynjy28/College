...
var
  new_Philosopher: Group; //для очередной записи
  f: file of Group; //файл данных
  i: integer; //счетчик цикла
begin
  adres:= ExtractFilePath(ParamStr(0));
  mRec.Cells[0, 0]:= 'Название';
  mRec.Cells[1, 0]:= 'Год создания';
  mRec.Cells[2, 0]:= 'Существует сейчас';
  mRec.Cells[3, 0]:= 'Первый релиз';
  mRec.Cells[4, 0]:= 'Жанр';

  //если файла данных нет, просто выходим:
  if not FileExists(adres + 'groups.dat') then exit;
  //иначе файл есть, открываем его для чтения и
  //считываем данные в сетку:
  try
    AssignFile(f, adres + 'groups.dat');
    Reset(f);
    ...
    while not Eof(f) do begin
      ...
      //добавляем в сетку новую строку, и заполняем её:
        ...
        mRec.Cells[0, mRec.RowCount-1]:= new_Philosopher.Name;
        mRec.Cells[1, mRec.RowCount-1]:= IntToStr(new_Philosopher.Foundation);
        if (new_Philosopher.Exist) then mRec.Cells[2, mRec.RowCount-1]:= 'Да'
        else mRec.Cells[2, mRec.RowCount-1]:='Нет';
        mRec.Cells[3, mRec.RowCount-1]:= FloatToStr(new_Philosopher.First_release);
        mRec.Cells[4, mRec.RowCount-1]:= new_Philosopher.Genre;
    end;
  finally
    CloseFile(f);
  end;
...
begin
  ...
  try
    ...
    for i:= 1 to mRec.RowCount-1 do begin
      ...
      new_Philosopher.Name:= mRec.Cells[0, i];
      new_Philosopher.Foundation:= StrToInt(mRec.Cells[1, i]);
      if (mRec.Cells[2, mRec.RowCount-1] = 'Да') then new_Philosopher.Exist := True
      else new_Philosopher.Exist := False;
      new_Philosopher.First_release:= StrToInt(mRec.Cells[3, i]);
      new_Philosopher.Genre:= mRec.Cells[4, i];
      //записываем их:
