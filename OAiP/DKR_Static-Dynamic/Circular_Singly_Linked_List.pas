...
var 
  Head,Sp,Spp: Pint;
  F: boolean;
  maxItems,switch,itemCount,DataType:integer;
...
begin
  ...
  repeat
    writeln(C^.value);
    writeln('Смотреть след.элемент (Нажмите Enter)');
    readln;
    C := C^.next;
  until C = Hed;
...
var 
  choice: char;
begin
  writeln('Выберите тип памяти для списка (1 - динамический, 0 - статический)');
  Readln(DataType);
  
  if DataType = 0 then
  begin
    Writeln('Введите максимальное число элементов в списке: ');
    Readln(maxItems);
    itemCount := 0;
  end;
